...
# helper functions
def heuristic(pt, END):  # manhattan distance
...
gscore[start] = 0
fscore = defaultdict(lambda: int(1e6))
fscore[start] = heuristic(start, end)

pq = queue.PriorityQueue()
pq.put((fscore[start], heuristic(start, end), start))
visited = set()
came_from = {}
...
while pq.qsize() != 0:
    ...
    for neighbour in neighbours:
        ...
        if temp_gscore < gscore[neighbour]:
            ...
            expected_dist = heuristic(neighbour, end)
            fscore[neighbour] = gscore[neighbour] + expected_dist
            pq.put((fscore[neighbour], expected_dist, neighbour))
